...
class TestFindFullPureStrategies:
    def test_terminal_node(self):
        ...
        result = find_full_pure_strategies(node, player="p0")
    ...
    def test_single_decision_node_player_owns(self):
        ...
        result = find_full_pure_strategies(root, player="p0")
        assert len(result) == 2
    ...
    def test_single_decision_node_player_does_not_own(self):
        ...
        root = DecisionNode("root", player="p1")
        left = TerminalNode("left", (S(1), S(2)))
        right = TerminalNode("right", (S(3), S(4)))
        root.add_child(left, "left")
        root.add_child(right, "right")
    ...
    def test_two_decision_nodes_same_player(self):
        ...
        root = DecisionNode("root", player="p0")
        left = TerminalNode("left", (S(1), S(2)))
        right = DecisionNode("n1", player="p0")
    ...
    def test_two_decision_nodes_different_players(self):
        ...
        root = DecisionNode("root", player="p0")
        left = TerminalNode("left", (S(1), S(2)))
        right = DecisionNode("n1", player="p1")
        a = TerminalNode("a", (S(3), S(4)))
        b = TerminalNode("b", (S(5), S(6)))
        right.add_child(a, "a")
        right.add_child(b, "b")
    ...
    def test_three_players(self):
        ...
        # t1 t2
        root = DecisionNode("root", player="p0")
        left = DecisionNode("n1", player="p1")
        right = TerminalNode("right", (S(1), S(2), S(3)))
        p2 = DecisionNode("n2", player="p2")
        ...
        p0_strats = find_full_pure_strategies(root, player="p0")
        assert len(p0_strats) == 2

        p1_strats = find_full_pure_strategies(root, player="p1")
        assert len(p1_strats) == 2

        p2_strats = find_full_pure_strategies(root, player="p2")
    ...
    def test_shared_information_set(self):
        ...
        root = DecisionNode("root", player="p0", information_set=info_set)
        other = DecisionNode("other", player="p0", information_set=info_set)
...
class TestFindReducedPureStrategies:
    def test_terminal_node(self):
        """Terminal node returns empty strategy."""
        node = TerminalNode("end", (S(1), S(2)))
        result = find_reduced_pure_strategies(node, player="p0")
        assert result == [Strategy({})]
    ...
    def test_single_decision_node(self):
        ...
        result = find_reduced_pure_strategies(root, player="p0")
        assert len(result) == 2
        assert Strategy({"root": "left"}) in result
        assert Strategy({"root": "right"}) in result
    ...
    def test_action_leads_to_more_decisions(self):
        ...
        full = find_full_pure_strategies(root, player="p0")
        reduced = find_reduced_pure_strategies(root, player="p0")
    ...
    def test_different_players(self):
        ...
        p0_reduced = find_reduced_pure_strategies(root, player="p0")
        p1_reduced = find_reduced_pure_strategies(root, player="p1")
    ...
    def test_chance_node(self):
        ...
        result = find_reduced_pure_strategies(chance, player="p0")
        assert result == [Strategy({})]
    ...
    def test_player_multiple_info_sets_different_branches(self):
        ...
        """
        root = DecisionNode("root", player="p0")
        o_node = DecisionNode("o", player="p1")
        m_node = DecisionNode("m", player="p1")
    ...
    def test_player_same_info_sets_different_branches(self):
        ...
        o_node = DecisionNode("o", player="p1", information_set=info_set)
        m_node = DecisionNode("m", player="p1", information_set=info_set)
        o_o = TerminalNode("oo", (S(2), S(1)))
        o_m = TerminalNode("om", (S(0), S(0)))
        m_o = TerminalNode("mo", (S(0), S(0)))
        m_m = TerminalNode("mm", (S(1), S(2)))
    ...
    def test_shared_info_set_siblings_with_subgames(self):
        ...
        """
        root = DecisionNode("root", player="p0")
        shared = InformationSet("shared", player="p1")
        n_a = DecisionNode("n_a", player="p1", information_set=shared)
        n_b = DecisionNode("n_b", player="p1", information_set=shared)

        r2_a_info = InformationSet("r2_a", player="p1")
        r2_b_info = InformationSet("r2_b", player="p1")
        deeper_a = DecisionNode("deeper_a", player="p1", information_set=r2_a_info)
        deeper_b = DecisionNode("deeper_b", player="p1", information_set=r2_b_info)
...
class TestCreatePayoffArray:
    def test_two_player_game(self):
        ...
        array, _ = create_payoff_array(root, {"p0": p0_strats, "p1": p1_strats})
    ...
    def test_single_player(self):
        ...
        array, _ = create_payoff_array(root, {"p0": p0_strats})
    ...
    def test_three_player_game(self):
        ...
        root = DecisionNode("root", player="p0")
        left = DecisionNode("n1", player="p1")
        right = DecisionNode("n2", player="p2")
        root.add_child(left, "L")
        root.add_child(right, "R")
        left.add_child(TerminalNode("t1", (S(1), S(2), S(3))), "U")
        left.add_child(TerminalNode("t2", (S(4), S(5), S(6))), "D")
        right.add_child(TerminalNode("t3", (S(7), S(8), S(9))), "U")
        right.add_child(TerminalNode("t4", (S(10), S(11), S(12))), "D")

        p0_strats = find_full_pure_strategies(root, player="p0")
        p1_strats = find_full_pure_strategies(root, player="p1")
        p2_strats = find_full_pure_strategies(root, player="p2")

        array, _ = create_payoff_array(
            root, {"p0": p0_strats, "p1": p1_strats, "p2": p2_strats}
        )

        assert array.shape == (2, 2, 2, 3)
        # p0=L: goes to n1 (p1 decides), p2's strategy is irrelevant
        assert tuple(array[0, 0, 0]) == (S(1), S(2), S(3))  # p0=L, p1=U -> t1
        assert tuple(array[0, 0, 1]) == (
            S(1),
            S(2),
            S(3),
        )  # p0=L, p1=U -> t1 (p2 irrelevant)
        assert tuple(array[0, 1, 0]) == (S(4), S(5), S(6))  # p0=L, p1=D -> t2
        assert tuple(array[0, 1, 1]) == (
            S(4),
            S(5),
            S(6),
        )  # p0=L, p1=D -> t2 (p2 irrelevant)
        # p0=R: goes to n2 (p2 decides), p1's strategy is irrelevant
        assert tuple(array[1, 0, 0]) == (S(7), S(8), S(9))  # p0=R, p2=U -> t3
        assert tuple(array[1, 0, 1]) == (S(10), S(11), S(12))  # p0=R, p2=D -> t4
        assert tuple(array[1, 1, 0]) == (
            S(7),
            S(8),
            S(9),
        )  # p0=R, p2=U -> t3 (p1 irrelevant)
        assert tuple(array[1, 1, 1]) == (
            S(10),
            S(11),
            S(12),
        )  # p0=R, p2=D -> t4 (p1 irrelevant)
    ...
    def test_chance_node(self):
        ...
        assert tuple(array[0, 0]) == (S(2), S(3))
    ...
    def test_with_reduced_strategies(self):
        ...
        # p0_full = find_full_pure_strategies(root, player="p0")
        p0_reduced = [
            Strategy({"root": "L"}),
        ...
        reduced_array, _ = create_payoff_array(root, {"p0": p0_reduced})

        assert reduced_array.shape == (3, 1)
